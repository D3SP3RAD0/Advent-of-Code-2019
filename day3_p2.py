...
def shift(p, command):
    try:
        ...
        if command[0] == "R":
            new_point[0] = p[0] + shift_amount
        elif command[0] == "L":
            new_point[0] = p[0] - shift_amount
        elif command[0] == "U":
            new_point[1] = p[1] + shift_amount
        elif command[0] == "D":
            new_point[1] = p[1] - shift_amount
        else:
...
def check_intersection(a, b, c, d):
    try:
        T_a = ((c[1]-d[1]) * (a[0]-c[0]) + (d[0]-c[0]) * (a[1]-c[1])) / ((d[0]-c[0]) * (a[1]-b[1]) - (a[0]-b[0]) * (d[1]-c[1]))
        T_b = ((a[1]-b[1]) * (a[0]-c[0]) + (b[0]-a[0]) * (a[1]-c[1])) / ((d[0]-c[0]) * (a[1]-b[1]) - (a[0]-b[0]) * (d[1]-c[1]))
    except ZeroDivisionError:
        return (0,0)
    if (0 <= T_a <= 1) and (0 <= T_b <= 1):
        return (a[0] + T_a*(b[0] - a[0]), a[1] + T_a*(b[1] - a[1]))
    else:
        return (0,0)
...
def main():
    ...
    wire_1 = [(0,0)] #stores points
    wire_2 = [(0,0)]

    for i, (command1, command2) in enumerate(zip(wire_1_commands, wire_2_commands)):
        current_point_1 = wire_1[i]
        wire_1.append(shift(current_point_1, command1)) #segment contains current as inital and shifted point as final
        current_point_2 = wire_2[i]
        wire_2.append(shift(current_point_2, command2))
    ...
    for i, point1 in enumerate(wire_1[1:]):
        for j, point2 in enumerate(wire_2[1:]):
            intersect = check_intersection(wire_1[i], point1, wire_2[j], point2)
            if intersect != (0,0):
                steps1 += abs(wire_1[i][0] - intersect[0]) + abs(wire_1[i][1] - intersect[1])
                steps2 += abs(wire_2[j][0] - intersect[0]) + abs(wire_2[j][1] - intersect[1])
                steps.append(steps1 + steps2)
            steps2 += abs(wire_2[j][0] - point2[0]) + abs(wire_2[j][1] - point2[1])
        steps2 = 0
        steps1 += abs(wire_1[i][0] - point1[0]) + abs(wire_1[i][1] - point1[1])
